package com.example.v_zhaihongyuan.gankio.utils;
...
public class ImageUtil {
    public static Uri saveImage(Context context, String url, Bitmap bitmap, ImageView imageView, String tag){
        //妹纸保存路径
        String imgDir = Environment.getExternalStorageDirectory().getPath() + "/GanKIO";
        //图片名称处理
        ...
        try {
            ...
            if (tag.equals("save")){
                if (compress){
                    Snackbar.make(imageView,"妹纸已经躺在你的图库里啦..小哥哥", Snackbar.LENGTH_SHORT).show();
                }else {
                    Snackbar.make(imageView,"妹纸拒绝了你的请求..小哥哥", Snackbar.LENGTH_SHORT).show();
                }
            }else{
                if (!compress){
                    Snackbar.make(imageView,"妹纸拒绝了你的请求.. 小哥哥", Snackbar.LENGTH_SHORT).show();
